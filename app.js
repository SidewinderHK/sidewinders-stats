// Sidewinders Stats - Multi-Season Hub v5.0
class SidewindersStats {
    constructor() {
        ...
        // Season definitions
        this.seasons = {
            current: { name: "2025-2026 (Current)", type: "full", file: "GameLog.csv", hasAnalysis: true, label: "2025-2026" },
            archive2526: { name: "2025-2026 Archive", type: "full", file: "GameLog25.csv", hasAnalysis: true, label: "2025-2026 Archive" },
    ...
    async init() {
        try {
            ...
            await this.loadSeason("current"); // default
    ...
    setupEventListeners() {
        ...
        $('#recalcBtn').off('click').on('click', () => {
            if (this.currentSeason && this.seasons[this.currentSeason].type === 'full') {
                ...
                // For summary, just refresh table (already computed)
                this.initLeagueTable();
            }
        });
    ...
    async loadSummarySeason(filename) {
        const csvText = await this.fetchCSV(filename);
        const summaryData = this.parseSummaryCSV(csvText);
        this.leagueTable = summaryData;
        this.players = summaryData.map(p => p.Player).sort();
        // No gameLog for summary
        this.gameLog = [];
    }
    ...
    parseSummaryCSV(csvText) {
        ...
        // Expected columns: Player, P, W, D, L, Gls, OG, Ast, Pts, ...
        const data = [];
        for (let i = 1; i < lines.length; i++) {
            ...
            // If points missing, compute from wins/draws
            if (points === 0 && (wins > 0 || draws > 0)) points = wins * 3 + draws;
            const ppg = games > 0 ? Math.round((points / games) * 10) / 10 : 0;
            const winPercent = games > 0 ? Math.round((wins / games) * 100 * 10) / 10 : 0;
            data.push({
                Player: player,
                Games: games,
                Wins: wins,
                ...
                OwnGoals: ownGoals,
                Assists: assists,
                Penalties: 0, // not available in summary
                TotalPoints: points,
                PPG: ppg,
    ...
    initLeagueTable() {
        ...
        // Player click only if full season (has analysis)
        if (this.seasons[this.currentSeason].hasAnalysis) {
            ...
            // For summary seasons, player names are not clickable (no analysis)
