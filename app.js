// Sidewinders Stats Hub - Multi-Season v6.5
class SidewindersStats {
    constructor() {
        ...
        this.seasons = {
            current:   { name: "Current Season",   file: "GameLog.csv",  label: "Current Season",   heading: "Current Season" },
            summer26:  { name: "Summer 2026",       file: "Summer26.csv", label: "Summer 2026",       heading: "Summer 2026" },
            archive2526: { name: "2025/26 Season",  file: "GameLog25.csv", label: "2025/26 Season",   heading: "2025/26 Season" },
            summer25:  { name: "Summer 2025",       file: "Summer25.csv", label: "Summer 2025",       heading: "Summer 2025" },
            season2425: { name: "2024/25 Season",   file: "Season24.csv", label: "2024/25 Season",   heading: "2024/25 Season" },
            season2324: { name: "2023/24 Season",   file: "Season23.csv", label: "2023/24 Season",   heading: "2023/24 Season" }
        };
    ...
    setupEventListeners() {
        $(document).off('click', '#seasonList .dropdown-item').on('click', '#seasonList .dropdown-item', (e) => {
            ...
            if (seasonId) this.loadSeason(seasonId);
        });

        $('#filterMinGames').off('change').on('change', () => this.refreshLeagueTable());

        $('#recalcBtn').off('click').on('click', () => {
            if (this.gameLog && this.gameLog.length > 0) this.calculateLeagueTable();
            this.refreshLeagueTable();
        });

        $('#resetSortBtn').off('click').on('click', () => this.resetSorting());
        $('#shareButton').off('click').on('click', () => this.shareCurrentPlayer());
    }
    ...
    setupExtendedStatsToggle() {
        ...
        const defaultState = (savedPref !== null) ? (savedPref === 'true') : isDesktop;
    ...
    async loadSeason(seasonId) {
        ...
        try {
            ...
            if (isFullLog) {
                ...
                // calculateLeagueTable is now self-contained and builds this.players itself
            ...
            // Debug logging – open the browser console to verify
            console.log(`[Sidewinders] ${season.label} loaded — ${this.gameLog.length} game rows, ${this.leagueTable.length} players in table.`);
        } catch (err) {
            console.error(`Failed to load season ${actualKey}:`, err);
            this.showError(`Could not load stats for ${season.name} (${season.file}).`);
            this.showLoading(false);
        }
    }
    ...
    parseCSV(csvText) {
        ...
        const cleanText = csvText.replace(/^\uFEFF/, '').replace(/\r/g, "").trim();
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (Object.keys(row).length > 0 && playerVal) data.push(row);
        }
        return data;
    }
    ...
    parseSummaryCSV(csvText) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            data.push({
                ...
                Goals: goals, OwnGoals: ownGoals, Assists: assists,
                Penalties: 0, TotalPoints: points,
                GPG: totalGames > 0 ? Math.round((goals / totalGames) * 100) / 100 : 0,
                APG: totalGames > 0 ? Math.round((assists / totalGames) * 100) / 100 : 0,
                PPG: totalGames > 0 ? Math.round((points / totalGames) * 100) / 100 : 0,
                WinPercent: totalGames > 0 ? Math.round((wins / totalGames) * 1000) / 10 : 0
            });
        }
        return data;
    }

    // ---- SELF-CONTAINED: builds this.players from this.gameLog itself ----
    calculateLeagueTable() {
        ...
        this.gameLog.forEach(game => {
            ...
            const result = resKey && game[resKey] ? game[resKey].toString().trim() : '';
        ...
        this.players = this.leagueTable.map(p => p.Player).sort((a, b) => a.localeCompare(b));
    }

    initLeagueTable() {
        if (this.dataTable) {
            try { this.dataTable.destroy(); } catch (e) { /* ignore */ }
            this.dataTable = null;
        }
        ...
        const columns = [
            ...
            { data: 'GPG', className: 'text-center', render: data => `<span class="gpg-value">${Number(data).toFixed(2)}</span>` },
            { data: 'APG', className: 'text-center', render: data => `<span class="apg-value">${Number(data).toFixed(2)}</span>` },
            { data: 'PPG', className: 'text-center', render: data => `<span class="ppg-value">${Number(data).toFixed(2)}</span>` },
        ...
        // Force a redraw so the initial render always paints correctly
        this.dataTable.columns.adjust().draw(false);
