// Sidewinders Stats Hub - Multi-Season v6.2
class SidewindersStats {
    ...
    setupEventListeners() {
        // Delegated listener for season selection dropdown
    ...
    async loadSeason(seasonId) {
        ...
        $(`#seasonList .dropdown-item[data-season="${seasonId}"]`).addClass('active');
        ...
        try {
            ...
            this.showError(`Could not load stats for ${season.name} (${season.file}). Make sure the file exists.`);
            this.showLoading(false);
        }
    }
    ...
    async loadFullSeason(filename) {
        ...
        this.players = [...new Set(this.gameLog.map(row => {
            const playerKey = Object.keys(row).find(k => k.toLowerCase() === 'player');
            return playerKey ? row[playerKey] : null;
    ...
    // Flexible column finder for robust summary CSV loading
    findColIndex(headers, possibleNames) {
        const cleanHeaders = headers.map(h => h.replace(/^"(.*)"$/, '$1').trim().toLowerCase());
    ...
    parseSummaryCSV(csvText) {
        ...
        const pIdx = this.findColIndex(headers, ['Player', 'Name']);
        const gIdx = this.findColIndex(headers, ['P', 'Games', 'Played', 'GP', 'MP']);
        ...
        const glsIdx = this.findColIndex(headers, ['Gls', 'Goals', 'G']);
        const ogIdx = this.findColIndex(headers, ['OG', 'Own Goals', 'OwnGoals']);
        const astIdx = this.findColIndex(headers, ['Ast', 'Assists', 'A']);
        const ptsIdx = this.findColIndex(headers, ['Pts', 'Points', 'Total Points', 'TotalPoints']);
        
        const data = [];
        for (let i = 1; i < lines.length; i++) {
            const values = this.parseCSVLine(lines[i]).map(v => v.replace(/^"(.*)"$/, '$1').trim());
            if (values.length === 0) continue;
